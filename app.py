...
def page_discovery():
    ...
    # Auto-fix: if no .NS/.BO suffix, try adding .NS
    ...
    if not price:
        st.error(f"❌ **{q}** not found. Check ticker symbol.")
        st.info("💡 Tips: NSE stocks → add .NS (e.g. RELIANCE.NS) · BSE → add .BO · US stocks → plain (AAPL)")
        return
