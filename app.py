...
/* Sidebar watchlist buttons - white text on dark */
...
def get_wl_tickers():
    """Get watchlist from persistent store (survives browser refresh)."""
    store = _wl_store()
    # Sync from DB if store is empty but DB has data
    if not store["tickers"]:
        try:
            conn = sqlite3.connect(DB)
            rows = conn.execute("SELECT ticker FROM watchlist ORDER BY added_at DESC").fetchall()
            conn.close()
            store["tickers"] = [r[0] for r in rows]
        except Exception:
            pass
...
def db_add(ticker, name=""):
    ...
    conn.execute("INSERT OR IGNORE INTO watchlist VALUES (?,?,?)", (t, name, now))
    conn.commit(); conn.close()
    # Update persistent store
    store = _wl_store()
    if t not in store["tickers"]:
        store["tickers"].insert(0, t)
...
if st.session_state.watchlist is None:
    # Load from persistent cache_resource (survives browser refresh)
    st.session_state.watchlist = list(get_wl_tickers())
    # Also add defaults if empty
...
@st.cache_resource
def _wl_store():
    """Global watchlist store — persists across browser sessions on same server."""
    rows = []
    try:
        conn = sqlite3.connect(DB)
        rows = conn.execute("SELECT ticker FROM watchlist ORDER BY added_at DESC").fetchall()
...
def safe_info(ticker):
    cache = _cache()
    key = f"info_{ticker}_{datetime.datetime.now().strftime('%Y%m%d%H')}"
    if key in cache:
        c = cache[key]
        if c.get("regularMarketPrice") or c.get("currentPrice"):
            return c
    info = {}
    try:
        ...
        try:
            raw = stk.info or {}
            if raw.get("regularMarketPrice") or raw.get("currentPrice"):
                info = raw
        except Exception:
            pass
        if not info.get("regularMarketPrice") and not info.get("currentPrice"):
            try:
                ...
                if p:
                    ...
                    info["regularMarketPreviousClose"] = float(pv) if pv else None
                    info["shortName"] = ticker
                    info["currency"]  = getattr(fi, "currency", "INR")
            except Exception:
                pass
        if not info.get("regularMarketPrice") and not info.get("currentPrice"):
            try:
                ...
                if not df.empty:
                    close = df["Close"]
                    info["regularMarketPrice"]        = float(close.iloc[-1])
                    info["regularMarketPreviousClose"] = float(close.iloc[-2]) if len(close)>=2 else None
                    info["shortName"] = ticker; info["currency"] = "INR"
            except Exception:
                pass
    except Exception:
        pass
...
def page_discovery():
    ...
    if not price and q and "." not in q:
        alt_info = safe_info(q + ".NS")
        alt_price = sf(_v(alt_info, "regularMarketPrice", "currentPrice"))
        if alt_price:
            info = alt_info; price = alt_price
            st.session_state.search_ticker = q + ".NS"
            st.session_state.search_data   = {"info": alt_info, "hist": safe_hist(q+".NS","6mo")}
    if not price:
        st.error(f"Could not fetch data for {q}. Check ticker symbol.")
        st.info("NSE: add .NS (e.g. RELIANCE.NS) · BSE: add .BO · US: AAPL")
        return
...
def page_watchlist():
    st.markdown(
        "<div class='page-header'>"
        "<div class='ph-title'>📋 My Watchlist</div>"
        "<div class='ph-sub'>Tap any stock to view full analysis</div>"
        "</div>", unsafe_allow_html=True)

    wl = st.session_state.watchlist

    if not wl:
        st.info("Watchlist empty. Go to Discovery Hub to add stocks.")
        if st.button("Go to Discovery Hub", type="primary"):
            st.session_state.page = "🔍 Discovery Hub"; st.rerun()
        return

    # If stock selected → show analysis
    sel = st.session_state.get("wl_selected", None)
    if sel and sel in wl:
        if st.button("← Back to Watchlist"):
            st.session_state.wl_selected = None; st.rerun()
        st.markdown("---")
        # Show stock analysis inline
        ...
        price = m.get("price") or m.get("ph")
        if not price:
            st.warning("Price unavailable"); return
        ...
        with h1:
            st.markdown(
                ...
                f"<div style='font-size:0.75rem;color:#64748b;'>{sel} · {m.get('exch','—')} · {m.get('sector','—')}</div>",
                unsafe_allow_html=True)
        with h2:
            st.markdown(
                f"<div style='text-align:right;'>"
                f"<div style='font-size:1.5rem;font-weight:800;font-family:JetBrains Mono,monospace;'>"
                f"{curr} {price:,.2f}</div>"
                f"<div style='font-size:0.85rem;font-weight:600;color:{cc};'>"
                f"{'▲' if chg>=0 else '▼'} {abs(chg):,.2f} ({chgp:+.2f}%)</div>"
                f"<div style='font-size:0.68rem;color:#94a3b8;'>AI: {total}/100 · Pio: {pio_s}/9</div>"
                f"</div>",
                unsafe_allow_html=True)

        st.markdown("---")
        if rsi_v and rsi_v > 70:
            st.error(f"RSI: {rsi_v:.1f} — Overbought. Caution.")
        elif rsi_v and rsi_v < 30:
            st.success(f"RSI: {rsi_v:.1f} — Oversold. Watch for bounce.")
        ...
        with k4: st.metric("RSI", f"{rsi_v:.1f}" if rsi_v else "—")
        with k5: st.metric("Piotroski", f"{pio_s}/9")
        with k6: st.metric("AI Score",  f"{total}/100")

        st.markdown("---")
        T1, T2, T3 = st.tabs(["📈 Chart","📋 Fundamentals","🤖 AI Verdict"])
        ...
        with T1:
            ...
            if vpa_d:
                st.info(f"Institutional Buying: {len(vpa_d)} signals. Last: {str(vpa_d[-1])[:10]}")

        with T2:
            sec("Valuation")
            c1,c2,c3,c4,c5 = st.columns(5)
            with c1: mc("Trailing PE", fn(m.get("pe"),1), "< 25 ideal")
            with c2: mc("Price/Book",  fn(m.get("pb"),2), "< 3 ideal")
            with c3: mc("ROE", fp(m.get("roe")), "> 15%",
                        "#16a34a" if m.get("roe") and m["roe"]>0.15 else "#dc2626" if m.get("roe") else "#94a3b8")
            with c4: mc("Net Margin", fp(m.get("pm")), "> 10%",
                        "#16a34a" if m.get("pm") and m["pm"]>0.10 else "#dc2626" if m.get("pm") else "#94a3b8")
            with c5: mc("FCF", fc(m.get("fcf")), "Positive=healthy",
                        "#16a34a" if m.get("fcf") and m["fcf"]>0 else "#dc2626" if m.get("fcf") else "#94a3b8")

            sec("Intrinsic Value")
            c1,c2,c3 = st.columns(3)
            with c1: mc("Graham", f"Rs.{iv.get('graham'):,.0f}" if iv.get("graham") else "N/A")
            with c2: mc("DCF",    f"Rs.{iv.get('dcf'):,.0f}"    if iv.get("dcf")    else "N/A")
            mos_v = iv.get("mos"); up_v = iv.get("up"); avg_iv = iv.get("avg")
            with c3: mc("Blended IV", f"Rs.{avg_iv:,.0f}" if avg_iv else "N/A",
                        f"MOS: {mos_v:.1f}%" if mos_v else "")
            if mos_v:
                if mos_v > 20:    st.success(f"Undervalued {mos_v:.1f}% — MOS exists!")
                elif mos_v > 0:   st.success(f"Marginally undervalued {mos_v:.1f}%")
                elif mos_v > -20: st.warning(f"Near fair value. Overvalued {abs(mos_v):.1f}%")
                else:             st.error(  f"Overvalued {abs(mos_v):.1f}%")
            ...
            with cb2:
                ...
                for pillar, pts in bd.items():
                    ...
                    st.markdown(
                        ...
                        f"border-radius:7px;padding:0.5rem 0.8rem;margin-bottom:0.3rem;'>"
                        f"<div style='display:flex;justify-content:space-between;'>"
                        f"<span style='font-size:0.78rem;font-weight:600;'>{pillar}</span>"
                        f"<span style='font-size:0.76rem;font-weight:700;color:{pc2};'>{pts}/20</span>"
                        f"</div>{pb_bar(pts/20*100, pc2)}</div>",
                        unsafe_allow_html=True)
            if total >= 75:   st.success(f"{grade} — Strong Buy. Piotroski {pio_s}/9.")
            elif total >= 45: st.warning(f"{grade} — Mixed signals.")
            else:             st.error(  f"{grade} — High risk. Avoid.")
            st.info("Research tool only — Not SEBI advice.")

        st.markdown("---")
        sec("My Notes")
        existing, last_upd = db_note_load(sel)
        note = st.text_area("", value=existing, height=180,
                            key=f"wlnote_{sel}", label_visibility="collapsed")
        nc1, nc2 = st.columns([3, 1])
        with nc1:
            if st.button("Save Note", key=f"wlsave_{sel}",
                         use_container_width=True, type="primary"):
                db_note_save(sel, note); st.success("Saved!")
        with nc2:
    ...
    # ── Stock List View ──────────────────────────────────────
    st.markdown(
        f"<div style='font-size:0.75rem;color:#64748b;margin-bottom:0.6rem;'>"
        f"{len(wl)} stocks · Tap to analyse</div>",
        unsafe_allow_html=True)
    ...
    st.markdown("<hr style='border:none;border-top:2px solid #e2e8f0;margin:4px 0 8px;'>",
                unsafe_allow_html=True)
    ...
    for ticker in wl:
        ...
        tabbr = ticker.replace(".NS","").replace(".BO","")[:3].upper()
        disp  = ticker.replace(".NS","").replace(".BO","")

        if price and prev and prev != 0:
            chgp  = (price - prev) / prev * 100
            cc    = "#16a34a" if chgp >= 0 else "#dc2626"
            cbg   = "#f0fdf4" if chgp >= 0 else "#fef2f2"
            chg_s = f"{'▲' if chgp>=0 else '▼'}{abs(chgp):.2f}%"
            p_str = f"Rs.{price:,.2f}"
        else:
            cc="#94a3b8"; cbg="#f8fafc"; chg_s="—"
            p_str = f"Rs.{price:,.2f}" if price else "—"

        c0, c1, c2, c3, c4 = st.columns([0.5, 3.5, 1.8, 0.7, 0.6])

        with c0:
            st.markdown(
                f"<div style='width:34px;height:34px;border-radius:7px;background:#e2e8f0;"
                f"display:flex;align-items:center;justify-content:center;font-size:0.68rem;"
                f"font-weight:700;color:#64748b;margin-top:4px;'>{tabbr}</div>",
                unsafe_allow_html=True)

        with c1:
            if st.button(f"**{disp}**  |  {name[:25]}",
                         key=f"wlsel_{ticker}", use_container_width=True):
                st.session_state.wl_selected = ticker; st.rerun()

        with c2:
            st.markdown(
                f"<div style='text-align:right;padding-top:4px;'>"
                f"<div style='font-size:0.88rem;font-weight:700;color:#0f172a;"
                f"font-family:JetBrains Mono,monospace;'>{p_str}</div>"
                f"<span style='font-size:0.73rem;font-weight:700;color:{cc};"
                f"background:{cbg};padding:2px 6px;border-radius:4px;'>{chg_s}</span>"
                f"</div>", unsafe_allow_html=True)

        with c3:
            if st.button("📊", key=f"dd_{ticker}", help="Deep Dive"):
        ...
        st.markdown("<hr style='border:none;border-top:1px solid #f1f5f9;margin:2px 0;'>",
                    unsafe_allow_html=True)

    st.caption(f"Yahoo Finance · {len(wl)} stocks")
