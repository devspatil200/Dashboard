...
def safe_info(ticker):
    ...
    try:
        ...
        info = stk.info or {}
        # If info empty, try fast_info as fallback
        if not info.get("regularMarketPrice") and not info.get("currentPrice"):
            try:
                fi = stk.fast_info
                info["regularMarketPrice"] = getattr(fi, "last_price", None)
                info["regularMarketPreviousClose"] = getattr(fi, "previous_close", None)
...
def safe_hist(ticker, period="1y"):
    ...
    try:
        ...
        if h.empty and "." not in ticker:
            h = yf.Ticker(ticker + ".NS").history(period=period)
...
def page_discovery():
    ...
    # Auto-fix: if no suffix, try adding .NS
    display_q = q
    if not price and "." not in q:
        q_ns = q + ".NS"
        info_ns = safe_info(q_ns)
        price_ns = sf(_v(info_ns, "regularMarketPrice", "currentPrice"))
        if price_ns:
            info = info_ns
            price = price_ns
            display_q = q_ns
            st.session_state.search_ticker = q_ns
            st.session_state.search_data = {"info": info_ns, "hist": safe_hist(q_ns, "6mo")}

    price = sf(_v(info, "regularMarketPrice", "currentPrice"))
    if not price:
        st.error(f"❌ **{display_q}** not found. Check ticker symbol.")
        st.info("💡 NSE → add .NS (e.g. RELIANCE.NS) · BSE → add .BO · US → plain (AAPL)")
        return
    q = display_q
