...
def safe_info(ticker):
    """Robust info fetch — tries multiple methods to get price data."""
    cache = _cache()
    key = f"info_{ticker}_{datetime.datetime.now().strftime('%Y%m%d%H')}"
    if key in cache:
        cached = cache[key]
        # Only use cache if it has price data
        if cached.get("regularMarketPrice") or cached.get("currentPrice"):
            return cached
        ...
        if not info.get("regularMarketPrice") and not info.get("currentPrice"):
            try:
                ...
                if price:
                    ...
                    info["regularMarketPreviousClose"] = float(prev) if prev else None
                    info["shortName"] = info.get("shortName") or ticker
                    info["currency"]  = getattr(fi, "currency", "INR")
                    # Try to get more data from history
                    try:
                        hist2 = stk.history(period="5d")
                        if not hist2.empty:
                            info["regularMarketVolume"] = int(hist2["Volume"].iloc[-1])
                            info["fiftyTwoWeekHigh"]    = float(hist2["High"].rolling(252, min_periods=1).max().iloc[-1])
                            info["fiftyTwoWeekLow"]     = float(hist2["Low"].rolling(252, min_periods=1).min().iloc[-1])
                    except Exception:
                        pass
            except Exception:
                pass
        
        # Method 3: Download for just price
        if not info.get("regularMarketPrice") and not info.get("currentPrice"):
            try:
                df = yf.download(ticker, period="5d", progress=False, auto_adjust=True)
                if not df.empty:
                    info["regularMarketPrice"]       = float(df["Close"].iloc[-1])
                    info["regularMarketPreviousClose"] = float(df["Close"].iloc[-2]) if len(df) >= 2 else None
                    info["regularMarketVolume"]      = int(df["Volume"].iloc[-1])
                    info["shortName"] = info.get("shortName") or ticker
                    info["currency"]  = "INR"
            except Exception:
                pass
...
def page_discovery():
    ...
    # Auto-retry with .NS suffix if price still missing
    if not price and "." not in q:
        alt_ticker = q + ".NS"
        alt_info = safe_info(alt_ticker)
        alt_price = sf(_v(alt_info, "regularMarketPrice", "currentPrice"))
        if alt_price:
            info  = alt_info
            price = alt_price
            st.session_state.search_ticker = alt_ticker
            st.session_state.search_data   = {"info": alt_info, "hist": safe_hist(alt_ticker, "6mo")}

    price = sf(_v(info, "regularMarketPrice", "currentPrice"))
    if not price:
        st.error(f"❌ **{q}** not found. Try again or check ticker.")
        st.info("💡 NSE: RELIANCE.NS · BSE: RELIANCE.BO · US: AAPL")
        return
